...
class ConfigInstallerTarballTest extends ConfigInstallerTestBase {
  ...
  protected function setUpSyncForm() {
    ...
    $this->drupalPostForm(NULL, ['files[import_tarball]' => __DIR__ . '/Fixtures/broken.tar.gz'], 'Save and continue');
    $this->assertText('Could not extract the contents of the tar file.');

    $this->drupalPostForm(NULL, ['files[import_tarball]' => __DIR__ . '/Fixtures/empty.tar.gz'], 'Save and continue');
    $this->assertText('The tar file contoins no files.');

    $this->drupalPostForm(NULL, ['files[import_tarball]' => __DIR__ . '/Fixtures/minimal-validation-fail.tar.gz'], 'Save and continue');
    $this->assertText('The tar file contoins no files.');
