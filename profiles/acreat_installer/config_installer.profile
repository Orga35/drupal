...
use Drupal\acreat_installer\Storage\SourceStorage;
 ...
 */
function acreat_installer_install_tasks_alter(&$tasks, $install_state) {
 ...
 * @see config_install_batch()
 */
function acreat_installer_install_uninstalled_profile_dependencies(array &$context, ConfigImporter $config_importer) {
  if (!array_key_exists('missing_profile_dependencies', $context)) {
    $profile = _acreat_installer_get_original_install_profile();
 ...
 * @see config_install_batch()
 */
function acreat_installer_config_import_profile(array &$context, ConfigImporter $config_importer) {
  $orginal_profile = _acreat_installer_get_original_install_profile();
  if ($orginal_profile) {
 ...
 * @see config_installer_install_tasks_alter()
 */
function acreat_installer_fix_profile() {
  ...
  if ($storage_comparer->hasChanges()) {
    ...
    _acreat_installer_switch_profile(_acreat_installer_get_original_install_profile());
    system_list_reset();
    $config_importer = new ConfigImporter(
      $storage_comparer,
      \Drupal::service('event_dispatcher'),
    ...
    _acreat_installer_switch_profile('config_installer');
    system_list_reset();
  }
}
 ...
 */
function _acreat_installer_switch_profile($profile) {
  global $install_state;
 ...
 */
function _acreat_installer_get_original_install_profile() {
